...
def create_webhook(roomId, target, webhook_name = "New Webhook"):
    ...
    spark_body = {
        ...
        "resource" : "messages",
        "event" : "created",
        "filter": "roomId={}".format(roomId),
    }
    
    #if (roomId != ""):
    #    {
    #        spark_body["filter"]: "roomId={}".format(roomId)
    #    }
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-r", "--roomid", help="Cisco Spark room Id", required=False
    )
    ...
    spark_roomid = args.roomid
    # print "Spark RoomId: " + str(spark_roomid)
    if (spark_roomid == None):
        spark_roomid = os.getenv("myhero_spark_roomid")
        # print "Env Spark RoomId: " + str(spark_roomid)
        if (spark_roomid == None):
            get_spark_roomid = raw_input("What is the Cisco Spark roomId? ")
            # print "Input Spark RoomId: " + str(get_spark_roomid)
            spark_roomid = get_spark_roomid
    # print "Spark RoomId: " + spark_roomid
    sys.stderr.write("Spark RoomId: " + spark_roomid + "\n")
    ...
    global_webhook_id = setup_webhook(spark_roomid, bot_url, "Global MyHero Demo Webhook")
